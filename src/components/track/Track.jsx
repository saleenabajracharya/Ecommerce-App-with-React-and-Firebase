import React from 'react';

export const Track = () => {
  return (
    <section>
      <div className="container mx-auto px-3 py-4 py-md-5">
        {/* Main */}
        <div className="row text-center">
          {/* Track 1 */}
          <div className="col-md-4 col-sm-9 col-9 p-3 ms-sm-5" style={{ marginLeft: window.innerWidth < 576 ? "40px" : "0px" }}>
            <div className="border border-secondary bg-light shadow-sm px-4 py-4 rounded-lg">
              <svg
                className="text-danger mb-3"
                width="48"
                height="48"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="#67a357"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"
                />
              </svg>
              <h2 className="h5 fw-medium text-dark">Premium T-Shirts</h2>
              <p className="text-muted">Our T-Shirts are 100% made of cotton.</p>
            </div>
          </div>

          {/* Track 2 */}
          <div className="col-md-4 col-sm-9 col-9 p-3 ms-sm-5" style={{ marginLeft: window.innerWidth < 576 ? "40px" : "0px" }}>
            <div className="border border-secondary bg-light shadow-sm px-4 py-4 rounded-lg">
              <svg
                className="text-danger mb-3"
                width="48"
                height="48"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="#67a357"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"
                />
              </svg>
              <h2 className="h5 fw-medium text-dark">Premium T-Shirts</h2>
              <p className="text-muted">Our T-Shirts are 100% made of cotton.</p>
            </div>
          </div>

          {/* Track 3 */}
          <div className="col-md-4 col-sm-9 col-9 p-3 ms-sm-5" style={{ marginLeft: window.innerWidth < 576 ? "40px" : "0px" }}>
            <div className="border border-secondary bg-light shadow-sm px-4 py-4 rounded-lg">
              <svg
                className="text-danger mb-3"
                width="48"
                height="48"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="#67a357"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"
                />
              </svg>
              <h2 className="h5 fw-medium text-dark">Premium T-Shirts</h2>
              <p className="text-muted">Our T-Shirts are 100% made of cotton.</p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};
